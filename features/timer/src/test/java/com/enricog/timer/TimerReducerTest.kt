...
class TimerReducerTest {
    ...
    @Test
    fun `test restartTime should set count timeInSeconds with the one in the segment`() =
        coroutineRule {
            val segment = Segment.EMPTY.copy(timeInSeconds = 10, type = TimeType.TIMER)
            val routine = Routine.EMPTY.copy(
                segments = listOf(segment)
    ...
    @Test
    fun `test nextStep should return same state when routine is completed`() = coroutineRule {
        val segment = Segment.EMPTY.copy(timeInSeconds = 10, type = TimeType.TIMER)
        val routine = Routine.EMPTY.copy(
            segments = listOf(segment)
        )
